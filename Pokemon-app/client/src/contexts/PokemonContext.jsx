...
export function PokemonProvider({ children }) {
  ...
  const fetchPokemons = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const response = await api.get(
        `/Pokemon${queryParams ? `?${queryParams}` : ""}`
      );
      dispatch(pokemonActions.fetchPokemonsSuccess(response.data));
    } catch (error) {
      dispatch(
        pokemonActions.fetchPokemonsFailure(
  ...
  const createPokemon = useCallback(async (pokemonData) => {
    ...
    try {
      ...
      dispatch(pokemonActions.createPokemonSuccess(response.data));
      return true;
    } catch (error) {
      dispatch(
        pokemonActions.createPokemonFailure(
          error.message || "Failed to create pokemon"
        )
      );
      return false;
    }
  ...
  const updatePokemon = useCallback(async (pokemonId, updateData) => {
    ...
    try {
      ...
      dispatch(
        pokemonActions.updatePokemonFailure(
          error.message || "Failed to update pokemon"
        )
      );
      return false;
    }
    ...
    try {
      ...
      dispatch(
        pokemonActions.deletePokemonFailure(
          error.message || "Failed to delete pokemon"
        )
      );
      return false;
    }
